...
async def test_kill_coordinator_during_op(manager: ManagerClient) -> None:
    ...
    6. Remove crashed coordinator after new coordinator was elected
    ...
    nodes = [await manager.server_add(config=config) for _ in range(4)]
    ...
    await manager.server_start(new_node.server_id, expected_error="Bootstrap failed. See earlier errors")
    ...
    await manager.server_stop_gracefully(coordinator_host.server_id)
    await manager.wait_for_host_down(other_nodes[-1].server_id, coordinator_host.server_id)
    await manager.remove_node(other_nodes[-1].server_id,coordinator_host.server_id)
