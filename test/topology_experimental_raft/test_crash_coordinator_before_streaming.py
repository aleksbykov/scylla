...
async def test_kill_coordinator_during_op(manager: ManagerClient) -> None:
    ...
    await manager.server_restart(coordinator_host.server_id, wait_others=1)
    logger.debug("Remove node with srv_id %s from node with srv_id %s because it was banned", working_srv_id, node_to_remove_srv_id)
    ...
    await manager.server_restart(node_to_replace_srv_id, wait_others=1)
    await manager.server_restart(coordinator_host.server_id, wait_others=1)
